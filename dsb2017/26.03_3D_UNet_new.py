...
def unet_fit(name, size, max_slices, augment=False, load_check=False,
             check_name=None):
    ...
    model = unet_model()
    lung_masks, nodule_masks = load_3d_data(start=0, end=1100,
                                            size_3d=max_slices, size=size)

    if augment:
        print('Using augmented data')
        scans_g, masks_g = augmentation(lung_masks, nodule_masks, 350)
        X_train, X_valid, y_train, y_valid = train_test_split(scans_g, masks_g,
                                                              test_size=0.2,
                                                              random_state=111)
        ...
        X_train, X_valid, y_train, y_valid = train_test_split(lung_masks,
                                                              nodule_masks,
                                                              test_size=0.2,
                                                              random_state=111)
        print('Training data shape: {}'.format(X_train.shape), '\n',
    ...
    model.fit(X_train, y_train, batch_size=1, nb_epoch=50, verbose=1,
              shuffle=True,
...
model = unet_fit('3D_UNet_newtry_full_cont', img_size, max_slices,
                 load_check=True, check_name='3D_UNet_newtry400pats')


# In[ ]:

lung_masks2, nodule_masks2 = load_3d_data(start=50, end=55,
                                          size_3d=max_slices, size=168)
