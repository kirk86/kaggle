...
# Model params
g_input_size = 1  # Random noise dimension coming into generator,
# per output vector
...
num_epochs = 30000
print_interval = 200
d_steps = 1  # 'k' steps in the original GAN paper.
# Can put the discriminator on higher training freq than generator
g_steps = 1

# ### Uncomment only one of these
# (name, preprocess, d_input_func) = ("Raw data", lambda data: data,
# lambda x: x)
(name, preprocess, d_input_func) = ("Data and variances", lambda data:
                                    decorate_with_diffs(data, 2.0),
                                    lambda x: x * 2)
...
for epoch in range(num_epochs):
    for d_index in range(d_steps):
        ...
        # but don't change params

        #  1B: Train D on fake
        d_gen_input = Variable(gi_sampler(minibatch_size, g_input_size))
        d_fake_data = G(d_gen_input).detach()  # detach to avoid training
        # G on these labels
        d_fake_decision = D(preprocess(d_fake_data.t()))
        d_fake_error = criterion(d_fake_decision, Variable(torch.zeros(1)))
        # zeros = fake
        d_fake_error.backward()
        d_optimizer.step()  # Only optimizes D's parameters; changes
        # based on stored gradients from backward()
