...
# simple NN one-hidden layer and 20 nodes
NHIDDEN = 20
...
W_out = tf.Variable(tf.random_normal([NHIDDEN, 1], stddev=1.0,
                                     dtype=tf.float32))
...
Wh = tf.Variable(tf.random_normal([1, NHIDDEN], stddev=STDEV,
                                  dtype=tf.float32))
bh = tf.Variable(tf.random_normal([1, NHIDDEN], stddev=STDEV,
                                  dtype=tf.float32))

Wo = tf.Variable(tf.random_normal([NHIDDEN, NOUT], stddev=STDEV,
                                  dtype=tf.float32))
bo = tf.Variable(tf.random_normal([1, NOUT], stddev=STDEV,
                                  dtype=tf.float32))
...
oneDivSqrtTwoPI = 1 / math.sqrt(2 * math.pi)  # normalisation factor
# for gaussian, not needed.
...
out_pi_test, out_sigma_test, out_mu_test = sess.run(get_mixture_coef(output),
                                                    feed_dict={x: x_test})
...
def generate_heatmap(out_pi, out_mu, out_sigma, x_heatmap_label,
                     y_heatmap_label):
    ...
    for k in range(0, K):
        for i in range(0, M):
            ...
            std = out_sigma[i, k]
            for j in range(0, N):
                z[N-j-1, i] += pi * custom_gaussian(y_heatmap_label[j],
                                                    mu, std)
...
z = generate_heatmap(out_pi_test, out_mu_test, out_sigma_test,
                     x_heatmap_label, y_heatmap_label)
